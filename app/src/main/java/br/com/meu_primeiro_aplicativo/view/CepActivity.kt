...
class CepActivity : AppCompatActivity() {
  ...
  private fun getConfigCepAndCall() {
    ...
    if (cep.isNotEmpty() && cep.length == 8) {
      ...
      Toast.makeText(this,"O CEP precisa conter 8 dígitos.",Toast.LENGTH_LONG).show()
    }
  }

  private fun callCepService(cep: String) {
    viewModel.getCep(cep = cep,
      ...
      if (cepResponse.isSuccessful){
        ...
        if (address?.erro != true) {
          tvDescriptionResult.text = String.format(resources.getString(R.string.mpa_address),
            address?.cep,
            address?.logradouro,
            address?.complemento,
            address?.bairro,
          ...
          Toast.makeText(this,resources.getString(R.string.mpa_error_cep), Toast.LENGTH_LONG).show()
        }
      } else {
        setVisibilityResult(false)
        Toast.makeText(this,resources.getString(R.string.mpa_error_cep), Toast.LENGTH_LONG).show()
      ...
      onError = {
        loading.visibility = View.GONE
        setVisibilityResult(false)
        Toast.makeText(this,resources.getString(R.string.mpa_error_comunication_api), Toast.LENGTH_LONG).show()
      })
  }
